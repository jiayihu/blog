...
Recently I've been dealing with Angular 2+ AOT compilation in Webpack very often and I've always found it an exhausting activity.
...
Despite we're almost at [Angular 5](https://github.com/angular/angular/blob/master/CHANGELOG.md#500-beta0-2017-07-19), in my opinion, the AOT compilation is still very experimental and a journey of try & error, digging into Angular source code and Github issues until you succeed or give up.  
...
1. Import and instantiate `AOTPlugin`. We'll use this plugin only in production to generate the JS bundle distributed and actually used by the users.  
   ...
   The plugin requires at least the path of the `tsconfig.json` used in production and the path to `app.module` file, where `AppModule` is exported. The latter must be absolute, whereas the former can be relative.

   You can read the package README for the [complete list of available options](https://github.com/angular/angular-cli/blob/5c4c5b488940cec13ed2c41e7a7031889f4f9b2f/packages/%40ngtools/webpack/README.md#options).
...
Note we're including `node_modules` because the Angular compiler will use TS files located in `node_modules`. This files are are called [*.ngfactory.ts](http://blog.mgechev.com/2016/08/14/ahead-of-time-compilation-angular-offline-precompilation/#inside-ngfactoryts) and the linked articles provide a clear oerview of their purpose.
...
1. We're importing `enableProdMode`, which will disable development behaviours. For example the application won't warn about [expression has changed error](https://github.com/angular/angular/issues/6005).

2. We're using `import { platformBrowser } from '@angular/platform-browser'` instead of `import { platformBrowserDynamic } from '@angular/platform-browser-dynamic'`. This will avoid including the JIT compiler (abot 2MB uncompressed) in the final bundle and it will instead make use of the AOT compilation files, included by `@ngtools/webpack`.
...
Now you're finally ready to run the AOT compilation! 🎉  This is an example of the npm script:

```json
"build": "cross-env NODE_ENV=production webpack"
```

Actually... if you try the compilation you're likely to run into a lot of scary red errors. That's because you still have to make your application code AOT ready.
...
2. **Typescript type checking fails**. The AOT compilation will also throw if a template is using a property/method which don't exist in the component class or if the types don't match. This doesn't happen with the JIT/runtime compilator.

3. **Logic errors** like dependecy cycles.

As you can see the latter 2 categories are actually useful errors, whereas the first one is just an hassle.
...
Before AOT compilation the `@angular` package had a size of 3.13MB uncompressed, whereas after the compilation it has been reduce to 1.29MB uncompressed. Still big, but surely better.
...
Another difference can be seen on the right side of the images. The AOT compiled application uses `.ngfactory.ts` files, produced by the AOT compiler, which replace the HTML templates and avoid runtime compilation.

Thanks to those statically compiled files, the initial loading time has been reduced of about 70%.
...
After the previous steps you should finally have an AOT compiled application. The most difficult step is get through all the compiler errors.  
...
Hopefully this article will also help you see some light in the dark. 
...
So far we covered AOT compilation with an Angular *application*, in a next post we'll see how to distribute AOT compatible *UI libraries* without Webpack and which unique requirements they must respect.
