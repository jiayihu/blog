...
function getArticlesJSON() {
  return new Promise((resolve, reject) => {
    const jsonPath = path.join(__dirname, 'gh-comments.json');
 ...
 */
function saveArticlesJSON(createdIssues) {
  return new Promise((resolve, reject) => {
    const jsonPath = path.join(__dirname, 'gh-comments.json');
 ...
 * @param {Function} options.getIssue Return the issue data based on the article
 ...
 */
function ghComments(articles, options) {
...
module.exports = function() {
  return (files, metalsmith, done) => {
    ...
    const articles = Object.keys(files)
      .filter(filepath => /articles\/.+\.html/.test(filepath))
      .map(filepath => {
    ...
    ghComments(articles, {
      ...
      idProperty: 'filepath',
      getIssue(article) {
        const articleUrl = url.resolve(metalsmith._metadata.siteurl, article.path);
        return {
          title: `Comments: ${article.title}`,
          body: `
            This issue is reserved for comments to [${article.title}](${articleUrl}) with Webpack.
            Leave a comment below and it will be shown in the blog page.
            `,
          labels: ['comments'],
        };
      },
      ...
      .then(createdIssues => {
        Object.keys(createdIssues).forEach(filepath => {
          const issueId = createdIssues[filepath].issueId;
          const article = files[filepath];
